...
const MyFree: FunctionComponent<{ board: string }> = ({ board }) => {
  const [rows, setRows] = useState<Free[]>([])
  ...
  useEffect(() => {
    ...
    axios
      .get('http://163.44.181.194:1212/api/board/free/myPost')
  ...
  return (
    <>
      <Box sx={{ m: '30px 30px' }}>
        ...
        <p>
          본인이 {board === 'free' && '자유 게시판에'} 작성한 게시물들을 볼 수
          있습니다.
        </p>
      </Box>
      <motion.div
      ...
      >
        ...
        <DialogContent>{id && <DetailFree id={id} />}</DialogContent>
        <DialogActions>
          <Button
